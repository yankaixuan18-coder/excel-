...
3. 在该块下方**插入等高的空行**。
4. 把上一周的内容写进新行（整行复制）：
   - **公式**：行号自动下移（`=H5/G5` → `=H11/G11`，复刻手动复制粘贴的效果），且**永不被覆盖**；
   - **常量**（SKU/ASIN 等文字）：原样照抄；
   - **数据列**：按 ASIN + 站点从数据表自动填（见下），没配数据表则留空等你填；
   - **BC 及之后的备注列**（比价方案/效果/改动内容）：清空；
   - **日期列**：写新日期（与原块相同相对行），并竖向合并。
5. 默认 **dry-run 只预览**，确认后加 `--apply` 才真正写入。

另外支持：
- **自动填数**：每周导出数据表 xlsx，按 `ASIN；国家` 匹配自动填入数据列（站点按标签名前缀如 `CA外卖包`→加拿大 推断）。详见 [docs/数据填充.md](docs/数据填充.md)。
- **多表多标签**：每个标签页配一个 `[[targets]]`，`python run.py run --all` 一次跑完。
